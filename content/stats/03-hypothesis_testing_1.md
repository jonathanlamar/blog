...
2. The _Type I Error_ associated to $\psi_n$ is $$ \alpha*{\psi_n}(\theta) =
   \mathbb{P}*\theta(\psi_n(X_1, X_2, \ldots, X_n) = 1 \mid
   ...
   guarantee.
3. The _Type II Error_ associated to $\psi_n$ is $$ \beta*{\psi_n}(\theta) =
   \mathbb{P}*\theta(\psi_n(X_1, X_2, \ldots, X_n) = 0 \mid
...
1. An important point about hypothesis testing: While we always want to strike a
   ...
   \sup\{\alpha*\psi(\theta) \mid \theta\in \Theta_0\}. $$ We will often say a
   test \_rejects at level $\alpha$\*.
2. Often, we will not understand the distribution of our test statistic
   ...
   \limsup*{n\to\infty}\sup\{\alpha*{\psi_n}(\theta) \mid \theta\in \Theta_0\}.
   $$
   $$
...
hypothesis $\Theta_1=(0,1/2)\cup(1/2,1)$. Let $$ T*n =
\sqrt{n}\cdot\frac{\lvert\bar{X_n} - 1/2\rvert}{\sqrt{(1/2)(1-(1/2))}} =
2\sqrt{n}\lvert\bar{X_n} - 1/2\rvert $$ and let $$ \psi*{n,\alpha} =
\mathbf{1}(T*n > q*{\alpha/2}). $$ Then by the Central Limit Theorem (assuming
